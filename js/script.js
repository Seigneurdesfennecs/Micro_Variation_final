...
$(window).on('scroll', function () {
    ...
    let scrollTop_ralenti = scrollTop / 1;

    function setRotation(id, value) {
        document.querySelector(id).style.setProperty("transform", `rotate(${value}deg)`, "important");
    }

    setRotation('#vi', scrollTop);
    setRotation('#vs', scrollTop);
    setRotation('#vi1', scrollTop_ralenti);
    setRotation('#vs1', scrollTop_ralenti);
    setRotation('#vi2', scrollTop / 2);
    setRotation('#vs2', scrollTop / 2);
    setRotation('#vi3', scrollTop / 3);
    setRotation('#vs3', scrollTop / 3);
    setRotation('#vi4', scrollTop / 5);
    setRotation('#vs4', scrollTop / 5);

    let opacity = 1 - (scrollTop / 800);

 // L'opacité commence à 1 et diminue avec le scroll
...
$(window).on('mouseenter', function() {
    $('#principale').css('cursor', 'crosshair');
});

$(window).on('mouseleave', function() {
    $('#principale').css('cursor', 'default');
});
...
//}); 
 ...
 $(document).ready(function () {
    ...
    let elementCible = null; // L'élément en cours de déplacement

    $('.moove').on('mousedown', function (event) {
      isDragging = true;
      elementCible = $(this); // On enregistre l'élément qu'on va déplacer
      elementCible.css('cursor', 'grabbing'); // Change le curseur
    });

    $(window).on('mousemove', function (event) {
      if (isDragging && elementCible) {
        elementCible.css({
          'top': event.pageY + 'px',
          'left': event.pageX + 'px'
        });
    ...
    $(window).on('mouseup', function () {
      isDragging = false;
      if (elementCible) {
        elementCible.css('cursor', 'grab'); // Retour au curseur normal
        elementCible = null; // On remet l'élément à zéro
      }
    });
  });
